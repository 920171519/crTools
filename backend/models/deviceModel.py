...
class Device(Model):
    ...
    required_vpn_display = fields.CharField(
        max_length=100, null=True, description="VPN显示名称")
    creator = fields.CharField(max_length=50, description="设备添加人")
    ftp_prefix = fields.BooleanField(
        default=False, description="连接FTP时是否需要输入前缀")
    support_queue = fields.BooleanField(default=True, description="是否支持排队占用")
    max_occupy_minutes = fields.IntField(
        null=True, description="最大占用时长（分钟），用于自动释放")
    ...
    device_type = fields.CharEnumField(
        DeviceTypeEnum, default=DeviceTypeEnum.TEST, description="设备归属类")
    form_type = fields.CharField(
        max_length=10, description="设备形态", default="未知")
    remarks = fields.TextField(null=True, description="设备备注信息")
    created_at = fields.DatetimeField(auto_now_add=True, description="创建时间")
    updated_at = fields.DatetimeField(auto_now=True, description="更新时间")
    # 连通性相关字段
    connectivity_status = fields.BooleanField(
        default=False, description="连通性状态")
    last_ping_time = fields.DatetimeField(
        null=True, description="最后一次ping检测时间")
    last_connectivity_check = fields.DatetimeField(
        null=True, description="最后一次连通性检查时间")
...
class DeviceUsage(Model):
    ...
    device = fields.OneToOneField(
        "models.Device", related_name="usage_info", description="设备")
    current_user = fields.CharField(
        max_length=50, null=True, description="当前占用人")
    ...
    status = fields.CharEnumField(
        DeviceStatusEnum, default=DeviceStatusEnum.AVAILABLE, description="设备状态")
    updated_at = fields.DatetimeField(auto_now=True, description="更新时间")
...
class DeviceInternal(Model):
    ...
    device = fields.ForeignKeyField(
        "models.Device", related_name="internal_info", description="设备", unique=True)
    ...
    def init_ports(self):
        ...
        if not self.port_status:
            self.port_status = {
                str(i): PortStatusEnum.INACTIVE for i in range(1, 21)}
...
class DeviceUsageHistory(Model):
    ...
    device = fields.ForeignKeyField(
        "models.Device", related_name="usage_history", description="设备")
...
class DeviceConfig(Model):
    ...
    device = fields.ForeignKeyField(
        "models.Device", related_name="config_items", description="关联设备")
    ...
    class Meta:
        ...
        unique_together = ("device", "config_param1",
                           "config_param2")  # 确保同一设备的参数组合不重复
...
class DeviceShareRequest(Model):
    ...
    device = fields.ForeignKeyField(
        "models.Device", related_name="device_share_requests", description="关联设备")
    requester_employee_id = fields.CharField(
        max_length=20, description="申请人工号")
    requester_username = fields.CharField(max_length=50, description="申请人姓名")
    status = fields.CharField(
        max_length=20, default="pending", description="申请状态")
    request_message = fields.CharField(
        max_length=255, null=True, description="申请备注")
    decision_reason = fields.CharField(
        max_length=255, null=True, description="处理说明")
    processed_by = fields.CharField(
        max_length=50, null=True, description="处理人工号")
...
class DeviceAccessIP(Model):
    ...
    device = fields.ForeignKeyField(
        "models.Device", related_name="access_ips", description="关联设备")
